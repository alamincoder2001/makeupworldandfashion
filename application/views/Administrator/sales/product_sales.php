				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ product.total }}</td>
							<td>
								<a href="" v-on:click.prevent="editFromCart(product)"><i class="fa fa-edit"></i></a>
	...
	new Vue({
		...
		methods: {
			...
			productTotal() {
				...
				document.querySelector('#productTotal').value = this.selectedProduct.total;
			},
			onSalesTypeChange() {
			...
			async productOnChange() {
				...
				this.selectedProduct.total = parseFloat(parseFloat(this.selectedProduct.Product_SellingPrice) * parseFloat(this.selectedProduct.quantity)).toFixed(2);
			...
			editFromCart(product) {
				this.selectedProduct = {
					Product_SlNo: product.productId,
					Product_Code: product.productCode,
					display_text: `${product.name} - ${product.productCode}`,
					ProductCategory_Name: product.categoryName,
